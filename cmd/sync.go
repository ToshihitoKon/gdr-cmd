...
func init() {
	...
	syncCmd.Flags().BoolVar(&syncChecksum, "checksum", false, "サイズが同じファイルも MD5 を比較し、内容が異なれば転送する (低速だが確実)")
...
type entry struct {
	...
	isGoogleDoc bool
	// md5 は Drive 側の MD5 チェックサム (--checksum 比較に使う。ローカル側は空)。
	md5 string
}
...
func syncLocalToDrive(ctx context.Context, localRoot, driveRoot string) error {
	...
	for _, rel := range sortedKeys(srcTree) {
		...
		// --checksum 時は、サイズ一致で mtime 上はスキップ対象でも MD5 を比較し、
		// 内容が異なれば転送する (サイズ衝突の取りこぼしを防ぐ)。
...
func syncDriveToLocal(ctx context.Context, driveRoot, localRoot string) error {
	...
	for _, rel := range sortedKeys(srcTree) {
		...
		transfer := needsTransfer(se.size, se.modTime, exists, de.size, de.modTime)
		// --checksum 時は、サイズ一致で mtime 上はスキップ対象でも MD5 を比較し、
		// 内容が異なれば転送する。Drive 側の md5 とローカルファイルを突き合わせる。
		if !transfer && syncChecksum && exists && se.size == de.size {
...
// contentDiffers はローカルファイルの MD5 が driveMD5 と異なるかを返す
// (--checksum での内容比較に使う)。driveMD5 が空の場合は Drive 側に
// チェックサムが無い (フォルダや一部の特殊ファイル) ため、比較不能として
// 「差なし (false)」を返し、サイズ/mtime の判定に委ねる。
func contentDiffers(localPath, driveMD5 string) (bool, error) {
	...
	if _, err := io.Copy(h, f); err != nil {
		return false, fmt.Errorf("%s の MD5 計算に失敗しました: %w", localPath, err)
	}
